...
def read_word_feat(path_word_vector, path_feat_vector):
    ...
    with open(path_word_vector, encoding="UTF-8") as f:
        for line in f:
            now_line += 1
...
def handle_sourcefeat(word, word_vec, feat_vec):
    ...
    if word_numpy is not None and feat_numpy is not None:
        ...
        # vec_numpy /= count
        vec_numpy = np.add(word_numpy, feat_numpy)
        vec_numpy = np.divide(vec_numpy, count)
        return vec_numpy
        ...
        vec_numpy = np.divide(feat_numpy, feat_count)
        return vec_numpy
...
def cal_error(sample_number, sample_k, word_vec, feat_vec, word_list):
    ...
    for sample_num in range(sample_number):
        ...
        # sample_word_list = set(random.sample(word_list, sample_k))
...
if __name__ == "__main__":
    ...
    for sample_k in list_sample_k:
        ...
        error_avg = cal_error(1000, int(sample_k), word_vec, feat_vec, word_list)
