# @Author : bamtercelboo
# @Datetime : 2018/1/23 10:22
# @File : filterVocab_suda_richfeat_source_feat.py.py
# @Last Modify Time : 2018/1/23 10:22
# @Contact : bamtercelboo@{gmail.com, 163.com}

"""
    FILE :  filterVocab_suda_richfeat_source_feat.py.py
    FUNCTION : source + feat
"""
...
def handle_source_feat(data_dict=None, feat_dict=None, source_dict=None, corpus_dict=None, path_filtedVectors=None):
    ...
    for index, word in enumerate(data_dict):
        ...
        # calculate
        if isinstance(source_word,  np.ndarray):
            feat_source_sum_embedding = (feat_sum_embedding + source_word) / ((feat_ngram_num + 1) + F_num / count_word)
            word_context_vector = word_context_vector / (count_word * (feat_ngram_num + 1) + F_num)
            word_context_ngram_embed = feat_source_sum_embedding + word_context_vector
        else:
...
if __name__ == "__main__":
    ...
    path_filtedVectors = "./suda_0120/richfeat/suda_richfeat0120_filtedVectors_source_feat30%.txt"
